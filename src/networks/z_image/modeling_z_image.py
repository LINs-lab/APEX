...
from .transformer_zimage import ZImageTransformer2DModelWrapper
...
class GenTransformer(torch.nn.Module):
    ...
    def forward(self, x_t, t, c=None, tt=None, **kwargs):
        ...
        if self.aux_time_embed and tt is not None:
            tt_sign = tt.sign()
            tt_abs = tt.abs()
        ...
        output_list = self.transformer(**transformer_kwargs)[0]
...
class ZImage(torch.nn.Module):
    def __init__(
        ...
        if aux_time_embed:
            z_image_transformer.init_time_embed_2_weights()
    ...
    def forward(self, x_t, t, c=None, tt=None, **kwargs):
        # return self.transformer(x_t, t, c=c, **kwargs)
    ...
    def encode_prompt(self, prompt, image=None, do_cfg=True):
        if do_cfg:
            if self.model_type == 't2i':
                # Z-Image Pipeline.encode_prompt returns (prompt_embeds_list, negative_prompt_embeds_list)
                # where each is a List[Tensor(L_i, D)]
                prompt_embeds_list, neg_prompt_embeds_list = self.model.encode_prompt(
                    prompt=prompt,
                    negative_prompt=[""] * len(prompt),  # Default negative prompt
                ...
                # Pad both to the same maximum length for CFG compatibility
                # Manually pad to ensure same length
            ...
            if self.model_type == 't2i':
                # Without CFG, only encode positive prompts
                ...
                # Convert to batched tensor with manual padding
