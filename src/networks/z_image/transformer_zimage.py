...
class ZImageTransformer2DModelWrapper(ZImageTransformer2DModel):
    ...
    def init_time_embed_2_weights(self):
        """
        Applies Xavier (Glorot) initialization to
        all nn.Linear layers within self.t_embedder_2.
        """

        print("Applying Xavier (Glorot) initialization to t_embedder_2...")
        def _init_weights(m):
        ...
        self.t_embedder_2.apply(_init_weights)
    ...
    def forward(
        ...
        with torch.autocast(device_type=device.type, enabled=False):
            ...
            if target_timestep is not None:
                ...
                # t_emb_3 = self.t_embedder_3(target_t_high * self.t_scale)
                t_emb = t_emb + t_emb_2 * delta_t.unsqueeze(1)
        ...
        adv_idx = t < 0
        if adv_idx.any():
            cap_feat_adv_mask = torch.zeros(
                cap_feats.shape[0], dtype=torch.bool, device=cap_feats.device
            )
            start_idx = 0
            for batch_idx, seq_len in enumerate(cap_item_seqlens):
                if adv_idx[batch_idx]:
                    cap_feat_adv_mask[start_idx : start_idx + seq_len] = True
                start_idx += seq_len
            cap_feats[cap_feat_adv_mask] = cap_feats[cap_feat_adv_mask] * (-1)
